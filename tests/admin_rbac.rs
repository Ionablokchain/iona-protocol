...
use iona::rpc::rbac::{
    ClientIdentity, RbacChecker, RbacDenial, RbacPolicy, Role, required_role,
};
...
fn alice()  -> ClientIdentity { ClientIdentity { cn: Some("alice-operator".into()),    fingerprint: None } }
fn bob()    -> ClientIdentity { ClientIdentity { cn: Some("bob-auditor".into()),       fingerprint: None } }
fn carol()  -> ClientIdentity { ClientIdentity { cn: Some("carol-maintainer".into()),  fingerprint: Some("AA:BB:CC:DD".into()) } }
fn dave()   -> ClientIdentity { ClientIdentity { cn: Some("dave-multi".into()),        fingerprint: None } }
fn nobody() -> ClientIdentity { ClientIdentity { cn: Some("unknown-hacker".into()),    fingerprint: None } }
...
fn a1_auditor_allowed_on_auditor_endpoint() {
    let p = sample_policy();
    assert!(p.is_allowed(&bob(), &Role::Auditor),
...
fn a1_maintainer_allowed_on_maintainer_endpoint() {
    let p = sample_policy();
    assert!(p.is_allowed(&carol(), &Role::Maintainer),
...
fn a2_auditor_denied_on_operator_endpoint() {
    let p = sample_policy();
    assert!(!p.is_allowed(&bob(), &Role::Operator),
...
fn a2_auditor_denied_on_maintainer_endpoint() {
    let p = sample_policy();
    assert!(!p.is_allowed(&bob(), &Role::Maintainer),
...
fn a3_unknown_identity_gets_no_roles() {
    let p = sample_policy();
    assert!(p.roles_for(&nobody()).is_empty(),
...
fn a3_unknown_identity_denied_auditor() {
    let p = sample_policy();
    assert!(!p.is_allowed(&nobody(), &Role::Auditor),
...
fn a4_anonymous_identity_has_no_roles_in_strict_policy() {
    ...
    let p = sample_policy();
    let anon = ClientIdentity { cn: None, fingerprint: None };
    assert!(p.roles_for(&anon).is_empty(),
...
fn a5_correct_fp_no_cn_denied_when_entry_requires_cn() {
    ...
    assert!(p.roles_for(&fp_only).is_empty(),
...
fn a6_maintainer_passes_operator_check() {
    let p = sample_policy();
    assert!(p.is_allowed(&carol(), &Role::Operator),
...
fn a6_maintainer_passes_auditor_check() {
    let p = sample_policy();
    assert!(p.is_allowed(&carol(), &Role::Auditor),
...
fn a8_hot_reload_revokes_role() {
    ...
    let checker = RbacChecker::new(sample_policy());
    assert!(checker.check(&alice(), "/admin/snapshot").is_ok(),
        "alice should be operator before reload");

    // Swap to a policy that gives alice only auditor.
    let new_policy: RbacPolicy = toml::from_str(r#"
[[identities]]
cn    = "alice-operator"
roles = ["auditor"]
"#).unwrap();
...
fn a8_hot_reload_adds_role() {
    ...
    let initial: RbacPolicy = toml::from_str(r#"
[[identities]]
cn    = "eve"
roles = ["auditor"]
"#).unwrap();
    let checker = RbacChecker::new(initial);
    let eve = ClientIdentity { cn: Some("eve".into()), fingerprint: None };

    assert!(checker.check(&eve, "/admin/snapshot").is_err(),
        "eve must be denied operator endpoint initially");

    // Promote to operator.
    let promoted: RbacPolicy = toml::from_str(r#"
[[identities]]
cn    = "eve"
roles = ["operator"]
"#).unwrap();
    ...
    assert!(checker.check(&eve, "/admin/snapshot").is_ok(),
...
fn a9_unknown_admin_endpoint_requires_maintainer() {
    assert_eq!(required_role("/admin/some-new-endpoint"), Role::Maintainer,
...
fn a10_client_identity_display_both() {
    ...
    assert!(format!("{id}").contains("CN=node-maint"),
        "display must include CN");
    assert!(format!("{id}").contains("fp=AA:BB:CC"),
        "display must include fingerprint");
}

#[test]
fn a10_client_identity_display_unknown() {
    let id = ClientIdentity { cn: None, fingerprint: None };
...
fn a10_rbac_denial_display() {
    ...
    assert!(s.contains("/admin/snapshot"), "denial string must name the endpoint");
}
...
fn dave_multi_role_still_denied_maintainer() {
    let p = sample_policy();
    assert!(!p.is_allowed(&dave(), &Role::Maintainer),
