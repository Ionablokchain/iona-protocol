...
fn apply_delegate(
    ...
    if bal < amount {
        return Err(format!(
            "delegate: insufficient balance (have {bal}, need {amount})"
        ));
    }
...
fn apply_undelegate(
    ...
    if delegated < amount {
        return Err(format!(
            "undelegate: insufficient delegation (have {delegated}, need {amount})"
        ));
    }
...
fn apply_withdraw(
    ...
    if withdrawn == 0 {
        return Err(
            "withdraw: nothing to withdraw (unbonding not complete or no unbonding)".into(),
        );
    }
