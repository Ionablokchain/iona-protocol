...
use crate::economics::staking::{StakingError, StakingState, Validator as EconValidator};
...
pub fn try_apply_staking_tx(
    ...
    Some(match result {
        Ok(gas) => StakingTxResult {
            success: true,
            error: None,
            gas_used: gas,
        },
        Err(e) => StakingTxResult {
            success: false,
            error: Some(e.to_string()),
            gas_used: GAS_DEFAULT,
        },
    })
}
...
fn apply_delegate(
    ...
    staking
        .delegate(from.to_string(), val_addr.to_string(), amount, |addr| {
...
fn apply_withdraw(
    ...
    *kv.balances.entry(from.to_string()).or_insert(0) = kv
        .balances
        .get(from)
        .copied()
        .unwrap_or(0)
        .saturating_add(withdrawn as u64);
...
fn apply_register(
    ...
    if bal < params.min_stake {
        return Err(StakingError::InsufficientBalanceForMinStake(
            params.min_stake,
        ));
    }
    ...
    *kv.balances.entry(from.to_string()).or_insert(0) = (bal - params.min_stake) as u64;
    ...
    staking.delegations.insert(
        key,
        crate::economics::staking::Delegation {
...
fn apply_deregister(
    ...
    let _key = (from.to_string(), from.to_string()); // self-delegation key
    let external_delegations: u128 = staking
        .delegations
        .iter()
        .filter(|((delegator, validator), _delegation)| validator == from && delegator != from)
...
mod tests {
    ...
    fn test_delegate_success() {
        ...
        let delegation = staking
            .delegations
            .get(&("bob".to_string(), "alice".to_string()))
            .unwrap();
        assert_eq!(delegation.amount, 100_000);
        assert_eq!(
            staking.validators.get("alice").unwrap().total_stake,
            1_100_000
        );
    }
    ...
    fn test_register_validator() {
        ...
        assert_eq!(
            *kv.balances.get("charlie").unwrap(),
            100_000 - params.min_stake as u64
        );
    }
...
pub fn validate_staking_tx(payload: &str) -> Result<StakingTx, String> {
    ...
    match parts[1] {
        "delegate" if parts.len() >= 4 => Ok(StakingTx {
            kind: StakingTxKind::Delegate,
            from: String::new(),
            validator: Some(parts[2].to_string()),
            amount: parts[3].parse().ok(),
            commission_bps: None,
        }),
        "undelegate" if parts.len() >= 4 => Ok(StakingTx {
            kind: StakingTxKind::Undelegate,
            from: String::new(),
            validator: Some(parts[2].to_string()),
            amount: parts[3].parse().ok(),
            commission_bps: None,
        ...
        "register" if parts.len() >= 3 => Ok(StakingTx {
            kind: StakingTxKind::Register,
            from: String::new(),
            validator: None,
            amount: None,
            commission_bps: parts[2].parse().ok(),
        }),
        "deregister" => Ok(StakingTx {
            kind: StakingTxKind::Deregister,
            from: String::new(),
            validator: None,
            amount: None,
            commission_bps: None,
