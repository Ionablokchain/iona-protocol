...
use iona::rpc_limits::{validate_tx, new_request_id, RpcLimiter, RpcLimitResult};
...
#[command(name = "iona-node", version = "28.1.0", about = "IONA v28.1 production node")]
struct Args {
...
async fn main() -> anyhow::Result<()> {
    ...
    match profile {
        ...
        "dev" => {
            tracing::warn!("Running in dev profile — security gates are RELAXED. Do NOT use in production.");
        }
        other => {
            anyhow::bail!(
    ...
    // Block public exposure unless the operator explicitly opts in.
    {
        ...
        let is_public = !listen.starts_with("127.") && !listen.starts_with("[::1]") && listen != "localhost";
        if is_public && !args.unsafe_rpc_public {
            anyhow::bail!(
