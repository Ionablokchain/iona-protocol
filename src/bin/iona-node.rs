...
async fn main() -> anyhow::Result<()> {
    ...
    let ds_guard = Some(
        DoubleSignGuard::new(&dd.root().display().to_string(), &signer.public_key()).map_err(
            |e| {
    ...
    // Shared state
    let app = App {
        mempool: Arc::new(Mutex::new(iona::mempool::pool::Mempool::new(
...
async fn get_staking(State(app): State<App>) -> Json<serde_json::Value> {
    ...
    let unbonding: Vec<serde_json::Value> = Vec::<((String, String), (u128, u64))>::new()
        .iter()
    ...
    fn on_commit(
        ...
        if self.snap.enabled && self.snap.every_n_blocks > 0 {
            if cert.height % self.snap.every_n_blocks == 0 && cert.height != self.last_snap_height {
                let prev_h = iona::storage::snapshots::latest_snapshot_height(
