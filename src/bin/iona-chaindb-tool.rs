    ...
    match args.cmd {
        Cmd::Info => {
            ...
            let blocks: Vec<iona::rpc::eth_rpc::Block> =
                iona::rpc::chain_store::load_jsonl(&f.blocks).unwrap_or_default();
            let receipts: Vec<iona::rpc::eth_rpc::Receipt> =
                iona::rpc::chain_store::load_jsonl(&f.receipts).unwrap_or_default();
            let txs: Vec<iona::rpc::eth_rpc::TxRecord> =
                iona::rpc::chain_store::load_jsonl(&f.txs).unwrap_or_default();
            let logs: Vec<iona::rpc::eth_rpc::Log> =
                iona::rpc::chain_store::load_jsonl(&f.logs).unwrap_or_default();
            println!(
