...
fn main() {
    ...
    match pos.get(0).map(|s| s.as_str()) {
        ...
        Some("cert") => {
            match pos.get(1).map(|s| s.as_str()) {
                ...
                _ => {
                    ...
                    eprintln!("  cert status              Show current cert subject, expiry, fingerprint");
        ...
        Some("support-bundle") => {
            let data_dir = pos.get(1).map(|s| s.as_str()).unwrap_or("./data");
            let output   = pos.get(2).map(|s| s.as_str()).unwrap_or("./iona-support-bundle.tar.gz");
        ...
        Some("audit") => {
            match pos.get(1).map(|s| s.as_str()) {
                ...
                Some("export") => {
                    ...
                    let last_n: usize = pos.windows(2)
                        .find(|w| w[0] == "--last")
...
fn cmd_doctor(rpc: &str, data_dir: &str) {
    ...
    match ureq::get(&health_url).timeout(std::time::Duration::from_secs(3)).call() {
        Ok(resp) if resp.status() == 200 => pass("RPC health", &format!("{rpc}/health → 200 OK")),
        Ok(resp) => { fail("RPC health", &format!("HTTP {}", resp.status())); failures += 1; }
        Err(e)   => { fail("RPC health", &format!("unreachable: {e}")); failures += 1; }
    }
        ...
        .call()
    {
        Ok(resp) => {
            if let Ok(v) = resp.into_json::<serde_json::Value>() {
                let peers = v.get("peers")
                    .and_then(|p| p.as_u64())
                    .unwrap_or(0);
                if peers >= 5 {
                    pass("Peer count", &format!("{peers} peers connected"));
                } else if peers >= 3 {
                    warn("Peer count", &format!("{peers} peers — below recommended 5"));
                    warnings += 1;
                } else {
        ...
        Err(_) => { warn("Peer count", "could not fetch status (node may be starting)"); warnings += 1; }
    }
    ...
    for (name, port) in ports {
        ...
        match TcpListener::bind(format!("127.0.0.1:{port}")) {
            Ok(_)  => warn(name, &format!("port {port} is free — node may not be running")),
            Err(_) => pass(name, &format!("port {port} in use (expected for running node)")),
        }
    }
    ...
    if data_path.exists() {
            ...
            unsafe {
                if libc::statvfs(path_cstr.as_ptr(), stat.as_mut_ptr()) == 0 {
                    ...
                    if pct_free >= 20 {
                        ...
                        warn("Disk space", &format!("{free_gib} GiB free ({pct_free}%) — below 20%"));
                        warnings += 1;
                    } else {
                        fail("Disk space", &format!("{free_gib} GiB free ({pct_free}%) — CRITICAL"));
                        failures += 1;
                    }
                } else {
        ...
        { pass("Disk space", &format!("data dir exists: {data_dir} (space check requires Linux)")); }
    } else {
        fail("Disk space", &format!("data dir does not exist: {data_dir}"));
        failures += 1;
    }
    ...
    #[cfg(unix)]
    {
        ...
        for (path, expected_mode) in &key_files {
            ...
            if let Ok(meta) = std::fs::metadata(p) {
                ...
                if mode == *expected_mode {
                    ...
                    fail("Key perms", &format!("{path}: 0{mode:03o} (expected 0{expected_mode:03o})"));
                    failures += 1;
                }
            }
        }
        if !any_key {
            warn("Key files", "no key files found in data dir (node may not be initialized)");
            warnings += 1;
        }
        ...
        if let Ok(meta) = std::fs::metadata(data_dir) {
            ...
            if mode <= 0o700 {
                ...
                warn("Data dir perms", &format!("{data_dir}: 0{mode:03o} — should be 0700 or stricter"));
                warnings += 1;
            }
        }
    }
        ...
        .call()
    {
        Ok(resp) => {
            if let Ok(v) = resp.into_json::<serde_json::Value>() {
                let node_ts = v.get("block_time")
                    .and_then(|t| t.as_u64())
                    .unwrap_or(local_ts);
                let drift = (local_ts as i64 - node_ts as i64).abs();
                if drift <= 2 {
                    pass("Time drift", &format!("{drift}s (local vs last block time)"));
                } else if drift <= 10 {
                    warn("Time drift", &format!("{drift}s — above 2s (check NTP sync)"));
                    warnings += 1;
                } else {
                    fail("Time drift", &format!("{drift}s — CRITICAL (BFT requires < 2s)"));
                    failures += 1;
                }
            }
        }
        Err(_) => { pass("Time drift", "cannot check (node not responding — skipped)"); }
    }
    ...
    if Path::new(&wal_path).exists() {
        pass("Signing WAL", &format!("{wal_path} present — double-sign protection active"));
    } else {
        warn("Signing WAL", &format!("{wal_path} not found — will be created on first sign"));
        warnings += 1;
    }
    ...
    if failures == 0 && warnings == 0 {
        ...
        if failures > 0 { println!("  {} {failures} failure(s)", red("✗")); }
        if warnings > 0 { println!("  {} {warnings} warning(s)", yellow("⚠")); }
    }
    println!("══════════════════════════════════════════\n");
...
fn cmd_upgrade_check(rpc: &str) {
        ...
        .call()
    {
        Ok(resp) => {
            if let Ok(v) = resp.into_json::<serde_json::Value>() {
                let latest = v.get("tag_name").and_then(|s| s.as_str()).unwrap_or("unknown");
...
fn cmd_backup(data_dir: &str, output_dir: &str) {
    ...
    if let Err(e) = std::fs::create_dir_all(output_dir) {
        eprintln!("  {} Cannot create output dir {output_dir}: {e}", red("FAIL"));
        process::exit(1);
    }
    ...
    match status {
        Ok(s) if s.success() => {
            if let Ok(meta) = std::fs::metadata(&backup_path) {
                ...
                println!("  {} Backup created: {backup_path} ({size_mb} MiB)", green("✓"));
            } else {
...
fn cmd_restore(backup_path: &str, data_dir: &str) {
    ...
    println!("  {} RESTORE WILL OVERWRITE: {data_dir}", yellow("⚠  WARNING"));
    ...
    {
        if resp.status() == 200 {
            eprintln!("  {} Node appears to be running (port 9001 is responding).", red("FAIL"));
    ...
    match status {
        Ok(s) if s.success() => {
            println!("  {} Restore complete: {backup_path} → {data_dir}", green("✓"));
            println!();
            println!("  Next steps:");
...
fn cmd_keys_check(data_dir: &str) {
    ...
    let key_entries: &[(&str, &str, u32)] = &[
        ...
        ("keys.json",     "Validator signing key (plaintext — should be replaced with keys.enc)", 0o600),
    ...
    #[cfg(unix)]
    {
        ...
        for (filename, description, expected_mode) in key_entries {
            ...
            if !p.exists() {
                ...
                if *filename == "keys.enc" {
                    warn("keys.enc", &format!("not found at {path} (node may not be initialized)"));
                    warnings += 1;
                }
                continue;
            }
        ...
        if Path::new(data_dir).exists() {
            if let Ok(meta) = std::fs::metadata(data_dir) {
                let mode = meta.mode() & 0o777;
                if mode & 0o077 == 0 {
                    pass("data dir", &format!("0{mode:03o} (no group/world access) ✓"));
                } else if mode & 0o007 != 0 {
                    fail("data dir", &format!("0{mode:03o} — world-readable!  Run: chmod 700 {data_dir}"));
                    failures += 1;
                } else {
                    warn("data dir", &format!("0{mode:03o} — group-readable. Recommended: chmod 700 {data_dir}"));
                    warnings += 1;
                }
            }
        } else {
    ...
    if Path::new(&wal_path).exists() {
        ...
        warn("signing.wal", "not found — will be created on first block signing");
        warnings += 1;
    }
    ...
    for cert_path in &cert_paths {
        if !Path::new(cert_path).exists() { continue; }
        cert_checked = true;
        ...
        match output {
            ...
            Ok(_) => {
                warn("admin cert", &format!("{cert_path} — could not parse (openssl error)"));
                warnings += 1;
            }
            Err(_) => {
                warn("admin cert", "openssl not available — cannot check cert expiry");
                warnings += 1;
            }
        }
        break;
    ...
    if !cert_checked {
        warn("admin cert", "no admin TLS cert found — mTLS admin port may not be configured");
        warnings += 1;
    }
    ...
    if failures == 0 && warnings == 0 {
        ...
        if failures > 0 { println!("  {} {failures} failure(s) — fix before running as validator", red("✗")); }
        if warnings > 0 { println!("  {} {warnings} warning(s)", yellow("⚠")); }
    }
    println!("══════════════════════════════════════════\n");
...
fn cmd_cert_rotate(data_dir: &str, days: u32) {
    ...
    if Path::new(&cert_path).exists() {
        ...
        match std::fs::copy(&cert_path, &backup) {
            ...
            Err(e) => { eprintln!("  {} Cannot back up cert: {e}", red("FAIL")); process::exit(1); }
        }
    } else {
    ...
    if !Path::new(&ca_path).exists() {
        println!("\n  Generating CA...");
        let r1 = std::process::Command::new("openssl").args(["genrsa", "-out", &ca_key_path, "4096"]).status();
        let r2 = std::process::Command::new("openssl")
            .args(["req", "-new", "-x509", "-days", &days.to_string(),
                   "-key", &ca_key_path, "-out", &ca_path,
                   "-subj", "/CN=IONA-Admin-CA/O=IONA/OU=Validator"])
            .status();
        match (r1, r2) {
            ...
            _ => { eprintln!("  {} openssl CA generation failed", red("FAIL")); process::exit(1); }
        }
    } else {
    ...
    let r_key  = std::process::Command::new("openssl").args(["genrsa", "-out", &key_path, "4096"]).status();
    let r_csr  = std::process::Command::new("openssl")
        .args(["req", "-new", "-key", &key_path, "-out", &csr_path,
               "-subj", "/CN=iona-admin/O=IONA/OU=Validator"]).status();
    let r_sign = std::process::Command::new("openssl")
        .args(["x509", "-req", "-days", &days.to_string(),
               "-in", &csr_path, "-CA", &ca_path, "-CAkey", &ca_key_path,
               "-CAcreateserial", "-out", &cert_path]).status();
    ...
    match (r_key, r_csr, r_sign) {
        ...
        _ => { eprintln!("  {} Certificate signing failed", red("FAIL")); process::exit(1); }
    }
    ...
    if !reloaded {
        warn("Hot-reload", "PID file not found — restart node to activate new cert");
    }
...
fn cmd_cert_status(data_dir: &str) {
    ...
    for (path, label) in &[
        ...
        if !Path::new(path).exists() { warn(label, &format!("{path} not found")); continue; }
        let output = std::process::Command::new("openssl")
        ...
        match output {
            ...
            _ => { warn(label, &format!("{path} — openssl parse error")); }
        }
        println!();
    }
}
...
fn cmd_cert_reload(rpc: &str) {
    ...
    let output = std::process::Command::new("curl")
        .args([
            ...
            "--max-time", "10",
            "--cert", "/etc/iona/tls/operator.crt",
            "--key",  "/etc/iona/tls/operator.key",
            "--cacert", "/etc/iona/tls/ca.crt",
            &format!("{}/admin/cert/reload", rpc.trim_end_matches('/')),
        ])
        .output();
    ...
    match output {
        Ok(o) if o.status.success() => {
            ...
            if body.contains("\"ok\":true") {
                ...
                for field in &["new_subject", "new_fingerprint", "expires_in_s",
                               "overlap_active", "overlap_seconds", "rotation_count"] {
...
fn cmd_cert_status_rpc(rpc: &str) {
    ...
    match output {
        Ok(o) if o.status.success() => {
            ...
            for field in &["subject_cn", "fingerprint", "expires_in_s", "not_after_unix"] {
                if let Some(val) = extract_json_field(&body, field) {
                    println!("    {field:20} {val}");
                }
            }
        ...
        _ => {
            ...
            for path in &["/etc/iona/tls/admin-server.crt", "/etc/iona/tls/ca.crt"] {
                let res = std::process::Command::new("openssl")
                    .args(["x509", "-noout", "-subject", "-enddate",
                           "-fingerprint", "-sha256", "-in", path])
                    .output();
                if let Ok(o) = res {
                    println!("  {}:", path);
                    for line in String::from_utf8_lossy(&o.stdout).lines() {
...
fn cmd_support_bundle(rpc: &str, data_dir: &str, output_path: &str) {
    ...
    if let Ok(resp) = ureq::get(&format!("{rpc}/status"))
        .timeout(std::time::Duration::from_secs(3)).call()
    {
        if let Ok(v) = resp.into_json::<serde_json::Value>() {
            let _ = std::fs::write(
    ...
    for config_path in &[
        ...
        if !Path::new(config_path.as_str()).exists() { continue; }
        if let Ok(content) = std::fs::read_to_string(config_path) {
            let sanitized: String = content.lines().map(|line| {
    ...
    for audit_path in &[format!("{data_dir}/audit.log"), "/var/lib/iona/audit.log".into()] {
        if !Path::new(audit_path.as_str()).exists() { continue; }
        ...
        .timeout(std::time::Duration::from_secs(3)).call()
    {
        if let Ok(text) = resp.into_string() {
            let filtered = text.lines()
                .filter(|l| l.starts_with("iona_") || l.starts_with("# HELP iona_") || l.starts_with("# TYPE iona_"))
                .collect::<Vec<_>>().join("\n");
    ...
    println!("  Collecting system info...");
    {
        ...
        for (cmd, args) in &[
            ...
            if let Ok(o) = std::process::Command::new(cmd).args(args.as_slice()).output() {
                sysinfo.push_str(&format!("{cmd}:\n{}\n", String::from_utf8_lossy(&o.stdout)));
            }
        }
        if let Ok(o) = std::process::Command::new("df").args(["-h", data_dir]).output() {
            sysinfo.push_str(&format!("df {data_dir}:\n{}\n", String::from_utf8_lossy(&o.stdout)));
        }
    ...
    let tar = std::process::Command::new("tar")
        .args(["czf", output_path, "-C", "/tmp", &format!("iona-support-{}", std::process::id())])
        .status();
    ...
    match tar {
        ...
        _ => { eprintln!("  {} tar failed", red("FAIL")); process::exit(1); }
    }
}
...
fn cmd_rbac_check(rpc: &str, identity: &str, endpoint: &str) {
    ...
    match ureq::get(&format!("{rpc}/admin/rbac/check?identity={identity}&endpoint={endpoint}"))
        .timeout(std::time::Duration::from_secs(3)).call()
    {
        Ok(resp) => {
            let status = resp.status();
            if let Ok(v) = resp.into_json::<serde_json::Value>() { print_json(&v); }
            if status == 200 {
...
fn cmd_rbac_export(data_dir: &str) {
    for path in &[
        ...
        if !std::path::Path::new(path.as_str()).exists() { continue; }
        match std::fs::read_to_string(path) {
            Ok(content) => { println!("# Exported from: {path}\n{content}"); return; }
...
fn cmd_audit_tail(path: &str, follow: bool) {
    ...
    // Print last 20 lines first
    {
        ...
        for line in &lines[lines.len().saturating_sub(20)..] { println!("{line}"); }
    }
    ...
    loop {
        ...
        match reader.read_line(&mut buf) {
            ...
            Ok(_)  => { print!("{}", buf.trim_end_matches('\n')); println!(); }
